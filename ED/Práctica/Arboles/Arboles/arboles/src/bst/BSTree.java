...
public class BSTree<T extends Comparable<T>> {
	 ...
	 * borrar y la clave a borrar. Se asigna a la raíz dicho nodo y se devuelve 0 El
	 * método busca la clave por la derecha o por la izquierda dependiendo de si la
	...
	private BSTNode<T> removeNodeRecursivo(BSTNode<T> nodoraiz, T clave) {
		if (nodoraiz.getInfo().compareTo(clave) > 0) {// si la raiz es mayor que la clave, busca por la izquierda el
			...
			if (nodoraiz.getLeft() == null && nodoraiz.getRight() == null)// si la raiz no tiene hijos
				return null;
			else if (nodoraiz.getLeft() != null && nodoraiz.getRight() == null)// si la raiz sólo tiene un hijo
																				// izquierdo
	...
	private BSTNode<T> searchMaxClave(BSTNode<T> nodoraiz) {
		if (nodoraiz.getRight() != null)// busca por los hijos derechos hasta llegar al último
